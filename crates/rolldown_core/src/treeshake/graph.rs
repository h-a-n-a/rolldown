...
use crate::{treeshake::TreeshakeNormalModule, BundleResult, Graph, COMPILER, SWC_GLOBALS};

impl Graph {
  pub(crate) fn treeshake(&mut self) -> BundleResult<()> {
    let used_ids = self
  ...
  pub(crate) fn collect_all_used_ids(&mut self) -> BundleResult<FxHashSet<Symbol>> {
