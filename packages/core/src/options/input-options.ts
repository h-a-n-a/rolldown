...
function normalizeInput(input: InputOptions['input']): BindingInputOptions['input'] {
    if (input == null) {
        return {}
        ...
        return Object.fromEntries(input.map(src => {
...
export function normalizeExternal(option: InputOptions['external']): BindingInputOptions['external'] {
    if (option == null) {
        return { string: [] }
    }
    ...
    return {
        function: (specifier, importer, isResolved) => regexs.some(regex => regex.test(specifier)),
        string: strings,
    }
}


async function normalizePlugins(option: InputOptions['plugins']): Promise<BindingInputOptions['plugins']> {
    const plugins = await normalizePluginOption(option)
    const adapters = plugins.map(plugin => createBuildPluginAdapter(plugin))
    return adapters
}

export async function normalizeInputOptions(input_opts: InputOptions): Promise<BindingInputOptions> {
    const { input, treeshake, external, plugins, cwd, ...rest } = input_opts;

    // Make sure all fields of RollupInputOptions are handled.
    const _empty: never = undefined as unknown as NonNullable<typeof rest[keyof typeof rest]>;
    return {
