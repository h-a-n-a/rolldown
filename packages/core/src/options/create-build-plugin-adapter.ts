...
export function createBuildPluginAdapter(plugin: Plugin): BuildPluginOption {
    ...
    return {
        ...
        resolveId: !plugin.resolveId ? undefined : async (specifier, importer) => {
            ...
            const ret = await handler.call(context, specifier, importer, { assertions: {}, get isEntry() { return unimplemented() } });
            if (typeof ret === 'string') {
                ...
                if (ret?.external === 'absolute' || ret?.external === 'relative') {
                    throw new Error(`External module type {${ret.external}} is not supported yet.`)
                }
                return {
                    id: ret.id,
