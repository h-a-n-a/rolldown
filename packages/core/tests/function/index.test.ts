...
function runSamples() {
    const matched = globby.sync('./samples/**/_config.ts', { cwd: __dirname, absolute: true })
    const samples = matched.map((sampleConfigPath) => {
        const id = path.relative(process.cwd(), sampleConfigPath).split(path.sep).join('_');
        return {
...
async function runSampleTest(sampleConfigPath: string) {
    ...
    const { default: testConfig }: { default: TestConfig } = (await import(sampleConfigPath))
